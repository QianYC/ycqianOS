;========= set the base addr for the boot program

	org 	0x7c00
BaseOfStack	equ	0x7c00

;========= this is the 1st sector

BaseOfLoader	equ	0x1000
OffsetOfLoader	equ	0x00

RootDirSectors	equ	14	; num of sectors of root dir
FirstRootSector	equ	19
FirstFAT1Sector	equ	1
SectorBalance	equ	17

	jmp	short	Label_Start
	nop
	BS_OEMName	db	"ycqianOS"
	BPB_BytesPerSec	dw	512
	BPB_SecPerClus	db	1
	BPB_RsvdSecCnt	dw	1
	BPB_NumFATs	db	2
	BPB_RootEntCnt	dw	224
	BPB_TotSec16	dw	2880
	BPB_Media	db	0xf0
	BPB_FATSz16	dw	9
	BPB_SecPerTrk	dw	18
	BPB_NumHeads	dw	2
	BPB_HiddSec	dd	0
	BPB_TotSec32	dd	0
	BS_DrvNum	db	0
	BS_Reserved1	db	0
	BS_BootSig	db	0x29
	BS_VolID	dd	0
	BS_VolLab	db	"boot loader"
	BS_FileSysType	db	"FAT12   "

;========= set those registers

Label_Start:

	mov	ax,	cs
	mov	ds,	ax
	mov	es,	ax
	mov	ss,	ax
	mov	sp,	BaseOfStack

;========= clear screen
	
	mov	ax,	0600h
	mov	bx,	0700h
	mov	cx,	0
	mov	dx,	0184fh
	int	10h

;========= set focus

	mov	ax,	0200h
	mov 	bx,	0000h
	mov	dx,	0000h
	int	10h

;========= display boot msg

	mov	ax,	1301h	; display msg function
	mov	bx,	000fh	; display msg in highlight white
	mov	dx,	0000h	; cursor coordinate
	mov	cx,	10	; length of msg
	push	ax
	mov	ax,	dx
	mov	es,	ax
	pop	ax
	mov	bp,	StartBootMessage
	int	10h

;========= reset floppy

	xor	ah,	ah
	xor	dl,	dl	; floppy driver A
	int	13h

;========= search loader function

	mov	word[SectorNo],	FirstRootSector

Label_SearchInRootDir:

	cmp	word[RootDirSizeForLoop],	0
	jz	Label_LoaderNotFound
	dec	word[RootDirSizeForLoop]
	mov	ax,	00h	; prepare params for function Func_ReadOneSector
	mov	es,	ax
	mov	bx,	8000h
	mov	ax,	[SectorNo]
	mov	cl,	1
	call	Func_ReadOneSector
	mov	si,	LoaderFileName
	mov	di,	8000h
	cld
	mov	dx,	10h

Label_SearchForLoader:

	cmp	cx,	0
	jz	Label_GotoNextSectorInRootDir
	dec	dx
	mov	cx,	11

Label_CmpFileName:

	cmp	cx,	0
	jz	Label_FileNameFound
	dec	cx
	lodsb
	cmp	al,	byte[es:di]
	jz	Label_GoOn
	jmp	Label_Different

Label_GoOn:

	inc	di
	jmp	Label_CmpFileName

Label_Different:

	and	di,	0ffe0h
	add	di,	20h
	mov	si,	LoaderFileName
	jmp	Label_SearchForLoader

Label_GotoNextSectorInRootDir:

	add	word[SectorNo],	1
	jmp	Label_SearchInRootDir

Label_LoaderNotFound:

	mov	ax,	1301h
	mov	bx,	008ch
	mov	dx,	0100h
	mov	cx,	21
	push	ax
	mov	ax,	ds
	mov	es,	ax
	pop	ax
	mov	bp,	NoLoaderMessage
	int	10h
	jmp	$

;========= found loader file name in root dir

Label_FileNameFound:

	mov	ax,	RootDirSectors
	and	di,	0ffe0h
	add	di,	01ah
	mov	cx,	word[es:di]
	push	cx
	add	cx,	ax
	add 	cx,	SectorBalance
	mov	ax,	BaseOfLoader
	mov	es,	ax
	mov	bx,	OffsetOfLoader
	mov	ax,	cx

Label_GoOnLoadingFile:

	push	ax
	push	bx
	mov	ah,	0eh	; display a char
	mov	al,	'.'
	mov	bl,	0fh
	int	10h
	pop	bx
	pop	ax

	mov	cl,	1
	call	Func_ReadOneSector
	pop	ax
	call	Func_GetFATEntry
	cmp	ax,	0fffh
	jz	Label_FileLoaded
	push	ax
	mov	dx,	RootDirSectors
	add	ax,	dx
	add	ax,	SectorBalance
	add	bx,	[BPB_BytesPerSec]
	jmp	Label_GoOnLoadingFile

Label_FileLoaded:

	jmp	$

;========= read sector from floppy function
; ax: start sector num
; cl: nums of sector to be read
; es:bx: target buffer address
;============================================

Func_ReadOneSector:

	push	bp
	mov	bp,	sp
	sub	esp,	2
	mov	byte[bp-2],	cl
	push	bx
	mov	bl,	[BPB_SecPerTrk]
	div	bl
	inc	ah
	mov	cl,	ah
	mov	dh,	al
	shr	al,	1
	mov	ch,	al
	and	dh,	1
	pop	bx
	mov	dl,	[BS_DrvNum]

Label_GoOnReading:

	mov	ah,	2
	mov	al,	byte[bp-2]
	int	13h
	jc	Label_GoOnReading
	add	esp,	2
	pop	bp
	ret

;========= get FAT entry
; ah stores the fat No
;============================

Func_GetFATEntry:

	push	es
	push	bx
	push	ax
	mov	ax,	00
	mov	es,	ax
	pop	ax
	mov	byte[Odd],	0
	mov	bx,	3
	mul	bx
	mov	bx,	2
	div	bx
	cmp	dx,	0
	jz	Label_Even
	mov	byte[Odd],	1

Label_Even:

	xor	dx,	dx
	mov	bx,	[BPB_BytesPerSec]
	div	bx
	push	dx
	mov	bx,	8000h
	add	ax,	FirstFAT1Sector
	mov	cl,	2
	call	Func_ReadOneSector
	pop	dx
	add	bx,	dx
	mov	ax,	[es:bx]
	cmp	byte[Odd],	1
	jnz	Label_Even2
	shr	ax,	4

Label_Even2:

	and	ax,	0fffh
	pop	bx
	pop	es
	ret

;========= initialize temp variables

RootDirSizeForLoop	dw	RootDirSectors
SectorNo		dw	0
Odd			db	0

;========= messages to be shown

NoLoaderMessage:	db	"ERROR:No Loader Found"
LoaderFileName:		db	"LOADER BIN",0
StartBootMessage:	db	"Start Boot"

;========= fill the rest of the sector

	times	510-($-$$)	db	0
	dw	0xaa55
